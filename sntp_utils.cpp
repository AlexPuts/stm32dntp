...
void ntp_recv_and_respond(uint16_t dest_port, uint8_t src_ip[4], uint16_t src_port, const char *data, uint16_t len)
{
  ...
  uint32_t recvTime = htonl(rtclock.getTime() + STARTOFTIME);
    ...
    uint32_t transmitTime =htonl( rtclock.getTime() + STARTOFTIME);
...
((char)b <= 'Z')) ) 
{ 
// считываем значения, содержащие цифры, буквы и точку
buffer += (char)b; 
}
if ( (char)b == ',' ) return buffer; // заканчиваем читать, когда встречаем запятую
}
}
...
else if((rtclock.getTime()-Time_last_sync )> 300) mb.Hreg(mb_begin_offset + 0,'V'); //set marker V in 0 register of modbus, - time is NOT valid
}
}
...
void fill_modbus_time()
{
  ...
  // The day for the given time_GPS t
  mb.Hreg(mb_begin_offset  + 6,rtclock.day()); 
  ////weekday(t); // Day of the week for the given NO week day is needed at the moment
  // The month for the given time_GPS t
  mb.Hreg(mb_begin_offset  + 7,rtclock.month());
  // year(t); // The year for the given time_GPS t
  mb.Hreg(mb_begin_offset  + 8,rtclock.year());
