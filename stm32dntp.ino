...
uint8_t myip[] = { 192,168,0,125 }; //Default ip
uint8_t ntp_protocol = true, mbtcp_protocol = true, melsec_protocol = false, gps_enabled = true;
void rtc_sec_intr() { if (rtc_is_second()) uptime_sec_count++; }
...
void setup() { 
  ...
  if((chksum == (uint8_t)EEPROM.read(0)+(uint8_t)EEPROM.read(1)+(uint8_t)EEPROM.read(2)+(uint8_t)EEPROM.read(3))&&chksum>4&&chksum<1000)
  {//if chksum is OK use ip from "eeprom"
    uint8_t ip []= { (uint8_t)EEPROM.read(0), (uint8_t)EEPROM.read(1), (uint8_t)EEPROM.read(2), (uint8_t)EEPROM.read(3) };
    memcpy(myip, ip, 4);
  }
  for(int i = 0; i < 15; i++) mb.addHreg(i,0);
